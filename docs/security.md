...
1. **Generate** — Initial keys are created with `yarn keys generate` (CLI bootstrap) or auto-generated by the DB key store on first boot
...
```bash
yarn keys generate    # Generate new JWKS keys (RS256, ES256, EdDSA)
```
