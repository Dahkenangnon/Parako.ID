...
export interface OIDCLoginManagerOptions {
...
export class OIDCLoginManager {
  ...
  private readonly socialButtonContents = new Map<HTMLButtonElement, string>();
    ...
    socialButtons.forEach(button => {
      button.addEventListener('click', (e: Event) => {
        ...
        if (!this.socialButtonContents.has(buttonElement)) {
          this.socialButtonContents.set(buttonElement, buttonElement.innerHTML);
        }
  ...
  private setSubmissionControlsDisabled(disabled: boolean): void {
    ...
    socialButtons.forEach(button => {
      ...
      if (!disabled) {
        const originalContent = this.socialButtonContents.get(socialButton);
        if (originalContent !== undefined) {
          socialButton.innerHTML = originalContent;
        }
      }
    });
...
export function initializeOidcLoginPage(): void {
...
if (typeof document !== 'undefined' && typeof window !== 'undefined') {
  document.addEventListener('DOMContentLoaded', initializeOidcLoginPage);
}
