...
export interface ResetPasswordManagerOptions {
...
export class ResetPasswordManager {
  ...
  private calculatePasswordStrength(password: string): {
    ...
    if (password.length >= this.passwordPolicy.minLength) {
      ...
      requirements.push(`At least ${this.passwordPolicy.minLength} characters`);
    }
    ...
    return {
      percentage: requirements.length > 0 ? Math.min(strength, 75) : strength,
      requirements,
    };
  }
...
function runFallbackManager(): void {
  try {
...
/** Register the template bootstrap after its JSON state is present in the DOM. */
export function installResetPasswordBootstrap(
    ...
    if (dataElement) {
      try {
        ...
        const resetPasswordManager = new ResetPasswordManager({
          ...
          errorRecoveryTimeout: data.config?.errorRecoveryTimeout,
        });
