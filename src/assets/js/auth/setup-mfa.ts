/**
 * SetupMFAManager - Handles MFA setup form interactions and OTP input management
 *
 * Features:
 ...
 *
 * Translation Support:
 * The SetupMFAManager supports full internationalization through the `t()` function in Nunjucks templates.
 * Translation keys used:
 * - auth.enableMFA: Button text for enabling MFA
 * - auth.enablingMFA: Loading text during MFA setup
 * - auth.codeRequired: Validation message for empty OTP code
 * - auth.codeInvalid: Validation message for invalid OTP code length
 * - auth.errorRecovery: Message shown when session times out
 *
 * Usage in templates:
 * {{ t('auth.enableMFA', 'Enable Two-Factor Authentication') | tojson }}
 *
...
(function () {
  ...
  interface SetupMFAManagerOptions {
    config: SetupMFAConfig;
    translations?: Partial<TranslationStrings>;
    debug?: boolean;
    errorRecoveryTimeout?: number;
  }

  class SetupMFAManager {
    private config: SetupMFAConfig;
    ...
    constructor(options: SetupMFAManagerOptions) {
      ...
      this.log('SetupMFAManager initialized', {
        config: this.config,
     ...
     */
    private validateConfig(config: SetupMFAConfig): SetupMFAConfig {
    ...
    private setupFormSubmission(): void {
      ...
      this.form.addEventListener('submit', (e: Event) => {
        ...
        this.submitButton!.innerHTML = `
          ...
          ${this.getTranslation('enablingMFA')}
        `;
    ...
    private enableAllButtons(): void {
      ...
      if (this.submitButton) {
        ...
        this.submitButton.innerHTML = this.getTranslation('enableMFA');
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const dataElement = document.getElementById('___SETUP_MFA_STATE___');

    if (dataElement) {
      try {
        const data = JSON.parse(dataElement.textContent || '{}');

        const setupMFAManager = new SetupMFAManager({
          config: data.config || {
            codeLength: 6,
            autoFocus: true,
            enablePaste: true,
        ...
        console.error('[SetupMFAManager] Failed to initialize:', error);
      ...
      console.error('[SetupMFAManager] No configuration data found in DOM');
