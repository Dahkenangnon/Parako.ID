...
export interface CustomIdentifierFieldInfo {
  slot: number;
  key: string;
  name: string;
...
export class RegisterManager {
  ...
  private setupPasswordStrengthValidation(): void {
    ...
    this.passwordInput.addEventListener('input', () => {
      ...
      if (this.passwordPolicy.requireSymbols && !value.match(/[^A-Za-z0-9]/)) {
      ...
      if (strengthPercentage < 25) {
        this.passwordStrength!.className = 'h-full bg-red-500 dark:bg-red-600';
