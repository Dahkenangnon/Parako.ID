...
type AdminUsersFormDialog = Pick<DialogService, 'showAlert'>;
...
export class AdminUsersFormManager {
  private readonly config: UsersFormConfig;
  ...
  constructor(
    ...
    private readonly dialog: AdminUsersFormDialog | null = dialogService
  ) {
    this.config = config;
    this.translations = {
  ...
  private setupPasswordMatchValidation(): void {
    ...
    if (passwordField) {
      passwordField.addEventListener('input', () => this.checkPasswordMatch());
    }
  ...
  private async validateCreateForm(): Promise<boolean> {
    ...
    if (!email || !givenName || !familyName || !password || !confirmPassword) {
  ...
  private async showValidationError(
    ...
    if (this.dialog) {
      await this.dialog.showAlert(title, message, { variant: 'error' });
      return;
    }
...
function createDefaultConfig(isCreateForm: boolean): UsersFormConfig {
  return {
    isCreateForm,
...
export function initializeAdminUsersFormPage(
  dialog: AdminUsersFormDialog | null = dialogService
): void {
  const stateElement = document.getElementById('___ADMIN_USERS_FORM_STATE___');
  const isCreateForm = document.getElementById('createUserForm') !== null;
  const defaultConfig = createDefaultConfig(isCreateForm);

  try {
    const config = stateElement
      ? (JSON.parse(
          stateElement.textContent || '{}'
        ) as Partial<UsersFormConfig>)
      : {};
    ...
    new AdminUsersFormManager(defaultConfig, dialog).initialize();
  }
}
