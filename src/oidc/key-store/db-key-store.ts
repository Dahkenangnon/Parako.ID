...
import type { JWKWithMetadata } from './constants.js';
...
export class DBKeyStore implements IKeyStore {
  ...
  async initialize(tenantId?: string): Promise<void> {
    ...
    if (count === 0) {
      ...
      const inserted = await this.generateAndStoreKeys(tenant, true, true);
      if (!inserted) {
        const concurrentCount = await this.repository.countCurrent(tenant);
        ...
        this.logger.info(
          `Loaded ${concurrentCount} keys generated by another process`,
          { tenantId: tenant }
        );
      }
    } else {
  ...
  async getJWKS(tenantId?: string): Promise<{ keys: JWKWithMetadata[] }> {
    const tenant = tenantId ?? DEFAULT_TENANT;
  ...
  async getPublicJWKS(tenantId?: string): Promise<{ keys: JWKWithMetadata[] }> {
    const tenant = tenantId ?? DEFAULT_TENANT;
  ...
  async retireKey(kid: string, tenantId?: string): Promise<boolean> {
    const tenant = tenantId ?? DEFAULT_TENANT;
    const keys = await this.repository.findAll(tenant);
    ...
    if (
      ...
      !keys.some(
        key =>
          key.kid !== kid && key.status === 'active' && key.promoted !== false
      )
    ) {
      throw new Error('Cannot retire the last promoted active signing key');
    }

    const retired = await this.repository.retireByKid(tenant, kid);
    if (retired) {
      this.logger.info('Retired JWKS key', { tenantId: tenant, kid });
    }
    return retired;
  }
  ...
  private async generateAndStoreKeys(
    ...
    let inserted = true;
    if (initial) {
      inserted = await this.repository.insertInitial(newDocs);
    } else {
