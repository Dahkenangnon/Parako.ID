...
describe('PrismaActivityRepository', () => {
  ...
  describe('findMany (paginated)', () => {
    ...
    it('sorts by the public username field through the actor relation', async () => {
      await repo.create(
        makeActivity({ actor: { actor_type: 'user', username: 'zoe' } })
      );
      await repo.create(
        makeActivity({ actor: { actor_type: 'user', username: 'alice' } })
      );

      const result = await repo.findMany({}, { sort: { username: 1 } });

      expect(result.results.map(activity => activity.actor?.username)).toEqual([
        'alice',
        'zoe',
      ]);
    });

    it('filters by the shared timestamp range contract', async () => {
      await repo.create(
        makeActivity({ timestamp: new Date('2026-01-01T00:00:00Z') })
      );
      await repo.create(
        makeActivity({ timestamp: new Date('2026-06-15T12:00:00Z') })
      );
      await repo.create(
        makeActivity({ timestamp: new Date('2026-12-31T23:59:59Z') })
      );
      ...
      expect(result.results[0]?.timestamp).toEqual(
