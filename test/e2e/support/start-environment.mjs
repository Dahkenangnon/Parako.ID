...
async function startRp() {
  ...
  app.get('/login', async (req, res) => {
    ...
    const url = buildAuthorizationUrl(configuration, {
      ...
      state,
      nonce,
      ...(req.query.prompt === 'consent' ? { prompt: 'consent' } : {}),
    });
  ...
  app.get('/jwt-userinfo/callback', async (req, res) => {
    ...
    const jwks = await fetch(metadata.jwks_uri).then(jwksResponse =>
      jwksResponse.json()
    );
...
async function main() {
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify(
      {
        features: { oidc: { jwt_userinfo: { enabled: true } } },
        security: {
          protection: {
            rate_limiting: {
              enabled: true,
